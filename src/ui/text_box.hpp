...
public:
    Text_Box(const std::shared_ptr<Graphics_Context> context, const std::string text, const std::string font, int ptsize, const SDL_Colour font_color, ALIGN_X alignX = ALIGN_X::CENTER, ALIGN_Y alignY = ALIGN_Y::CENTER, SDL_Rect rect = SDL_Rect({0, 0, 0, 0}), SDL_Colour color = SDL_Colour({0, 0, 0, 0}), SDL_Colour border_color = SDL_Colour({0, 0, 0, 0}), bool hidden = false, int r = 0)
